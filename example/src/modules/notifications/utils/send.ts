...
export async function notifyInBulk<U extends User>(
	...
	notification: (
		...
		(PushNotification & { afterSent: () => Promise<void> }) | undefined
	>,
) {
	for (const user of users) {
		...
		if (notificationToSend) {
			...
			console.info(
				`[cron ${jobId} @ ${
					user.uid
				}] Sending notification ${JSON.stringify(notificationData)}`,
			);
...
export async function notify<U extends User>(
	...
	notification:
		| PushNotification
		| ((user: U) => MaybePromise<PushNotification>),
	...
	await Promise.all(
		subscriptions.map(async (subscription) => {
			...
			if (
				!serverCanSendNotificationToUser(
					subscription.owner,
					notif.data.channel,
				)
			) {
				console.info(
					`[${notif.data.channel} on ${
						notif.data.group ?? 'global'
					} @ ${owner.id}] Skipping since user has disabled ${
						notif.data.channel
					} notifications`,
				);
				return;
			...
			try {
				...
				await prisma.notification.create({
					data: {
						...
						...(notif.data.group
							? { group: { connect: { uid: notif.data.group } } }
							: {}),
					},
				});
