...
export async function scheduleNotification(
	...
	if (at.valueOf() <= Date.now() && !eager) {
		console.info(
			`[cron ${id}] Not scheduling notification in the past and eager is false`,
		);
		return false;
	}
	...
	if (at.valueOf() <= Date.now()) {
		...
		if (dryRun) {
			console.info(
				`[cron ${id}] (dry run) would schedule ${JSON.stringify(
					notification,
				)} to ${users.length} users (${users
					.map(({ uid }) => `@${uid}`)
					.join(' ')})`,
			);
			return false;
		...
		async () => {
			for (const user of users) {
				console.info(
					`[cron ${id} @ ${
						user.uid
					}] Sending notification (time is ${at.toISOString()})`,
				);
				const notificationToSend = await notification(user);
				if (notificationToSend) {
					const { afterSent, ...notificationData } =
						notificationToSend;
