...
export const TicketType = builder.prismaNode('Ticket', {
	...
	fields: (t) => ({
		...
		openToContributors: t.exposeBoolean('openToContributors', {
			nullable: true,
		}),
		openToApprentices: t.exposeBoolean('openToApprentices', {
			nullable: true,
		}),
		...
		remainingGodsons: t.int({
			async resolve({ godsonLimit, eventId }, _, { user }) {
				...
				if (managers.some(({ userId }) => user?.id === userId))
					return -1;
