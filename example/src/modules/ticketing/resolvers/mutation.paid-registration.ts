...
builder.mutationField('paidRegistration', (t) =>
	t.field({
		...
		async authScopes(_, { regId }, { user }) {
			...
			if (!registration)
				throw new GraphQLError("La réservation associée n'existe pas");

			if (creating) {
				// Check that the user can access the event
				if (
					!(await userCanAccessEvent(registration.ticket.event, user))
				)
				...
				if (
					...
					throw new GraphQLError(
						'Seul un·e manager peut donner cette place',
					);

				// Check that the ticket is still open
				if (
					registration.ticket.closesAt &&
					registration.ticket.closesAt.valueOf() < Date.now()
				)
		...
		async resolve(
			_,
			{ regId, beneficiary, paymentMethod, phone },
			{ user },
		) {
			const registration = await prisma.registration.findUnique({
				where: { id: regId },
			...
			if (paymentMethod === PaymentMethodPrisma.PayPal) {
				if (!paypalOrderId)
					throw new GraphQLError(
						"PayPal n'a pas donné d'identifiant de commande",
					);
...
async function pay({
	...
	switch (by) {
		...
		case 'PayPal': {
			if (!registrationId)
				throw new GraphQLError('Pas de réservation associée');
			return payEventRegistrationViaPaypal(
				registrationId,
				emailAddress ?? '',
			);
		}
