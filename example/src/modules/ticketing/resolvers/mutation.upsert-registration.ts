...
builder.mutationField('upsertRegistration', (t) =>
	t.prismaField({
		...
		async authScopes(
			_,
			{ ticketId, id, paid, authorEmail, beneficiary },
			{ user },
		) {
			const logDenial = async (
				...
				await log(
					...
					{
						message: why,
						args: { paid, authorEmail, beneficiary },
						...data,
					},
					id,
					user,
			...
			if (
				...
				!(
					user?.admin ||
					userCanManageEvent(ticket.event, user, {
			...
			if (creating) {
				...
				if (!(await userCanAccessEvent(ticket.event, user))) {
					await logDenial(
						"user can't access the event the ticket is for",
						{ ticket },
					);
					return false;
				}
				...
				if (!userCanSeeTicket(ticket, userWithContributesTo)) {
					await logDenial("user can't see the ticket", {
						ticket,
				...
				if (
					...
					await logDenial('only managers can provide this ticket', {
						ticket,
					});
					return false;
				}
				...
				const ticketAndRegistrations = await prisma.ticket.findUnique({
					...
					include: {
						...
						group: {
							include: {
								tickets: { include: { registrations: true } },
							},
						},
					},
				});
				...
				if (!userCanManageEvent(ticket.event, user, {})) {
					const registrationsByThisAuthor =
						ticketAndRegistrations!.registrations.filter(
							({ author, beneficiary }) =>
								author?.uid === user?.uid && beneficiary !== '',
						);
			...
			if (
				!user?.admin &&
				!userCanManageEvent(registration.ticket.event, user, {
					canVerifyRegistrations: true,
				})
			)
				return false;
		...
		async resolve(
			...
			const ticket = await prisma.ticket.findUniqueOrThrow({
				where: { id: ticketId },
				include: {
					event: { include: { beneficiary: true } },
					autojoinGroups: true,
				},
			});
			...
			if (
				(beneficiary || user) &&
				paid &&
				ticket.autojoinGroups.length > 0
			) {
				try {
			...
			if (creating) {
				...
				const pseudoID = registration.id
					.replace(/^r:/, '')
					.toUpperCase();
				const qrcodeBuffer = await qrcode.toBuffer(pseudoID, {
				...
				if (!recipient)
					throw new GraphQLError(
						'No recipient found to send email to.',
					);
				await sendMail(
					'booking',
					recipient,
