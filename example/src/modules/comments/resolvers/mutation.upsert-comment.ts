...
builder.mutationField('upsertComment', (t) =>
	t.prismaField({
		...
		async resolve(
			query,
			_,
			{ id, body, resourceId, inReplyToId },
			{ user },
		) {
			const connection:
				| {}
				| { articleId: string }
				| { documentId: string } = resourceId
				? {
						[splitID(resourceId)[0] === 'Document'
							? 'documentId'
			...
			const commentedOn:
				| { title: string; author: User | null }
				| undefined = comment.document
				? {
						title: comment.document.title,
						author: comment.document.uploader,
					}
				: comment.article
					? {
			...
			const commentUrl =
				...
				(comment.document
					? `/documents/${documentMajor}/${
						...
						}a/${comment.document.subject!.uid}/${
							comment.document.uid
						}/`
					: `/posts/${comment.article!.group.uid}/${
							comment.article!.uid
						}`) +
				`#comment-${comment.id.replace(
					TYPENAMES_TO_ID_PREFIXES.Comment + ':',
					'',
				)}`;
			...
			if (
				...
				await notify([commentedOn.author], {
					title: `@${comment.author?.uid ?? '???'} a commenté sur ${
						commentedOn.title ?? '???'
					}`,
					body: comment.body,
					data: {
