...
builder.mutationField('upsertBarWeek', (t) =>
	t.prismaField({
		...
		async resolve(
			query,
			_,
			{ id, startsAt, endsAt, description, groupsUids },
			{ user },
		) {
			// Check if ends at date is after starts at date
			if (endsAt < startsAt)
				throw new GraphQLError('Bar week ends before it starts.');
			...
			if (overlaps.length > 0)
				throw new GraphQLError(
					'Bar week overlaps with another bar week.',
				);
			const data = {
				startsAt,
				endsAt,
				description,
				uid: slug(
					`${groupsUids.join('-')}-${startsAt.getFullYear()}-${
						startsAt.getMonth() + 1
					}`,
				),
			};
