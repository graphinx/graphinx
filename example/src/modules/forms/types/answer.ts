	...
	fields: (t) => ({
		...
		section: t.prismaField({
			...
			resolve: (query, { question: { sectionId } }) =>
				prisma.formSection.findUniqueOrThrow({
					...query,
					where: { id: sectionId },
				}),
		...
		answerString: t.string({
			resolve: (answer, {}, { user }) => answerToString(answer, user),
		}),
		checkboxIsMarked: t.boolean({
			...
			async resolve(
				...
				return markedCheckboxes.some(
					(checkbox) => checkbox.id === createdById,
				);
			},
		}),
	}),
