	...
	validate: [
		[
			async ({ question, answer }) => {
				const { type } = await prisma.question.findUniqueOrThrow({
					where: { id: question },
				});
		...
		[
			async ({ question, answer }) => {
				const { type, scaleStart, scaleEnd } =
		...
		[
			async ({ question, answer }) => {
				const { type, options, allowOptionOther } =
					await prisma.question.findUniqueOrThrow({
						where: { id: question },
					});
				...
				if (answer.length === 0) return true;
				return (
					allowOptionOther || answer.every((a) => options.includes(a))
				);
			},
			{
		...
		[
			...
			{
				message:
					'Cette réponse doit être une date au format YYYY-MM-DD',
			},
		],
		[
			async ({ question, answer }) => {
