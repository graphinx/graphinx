...
export const FormType = builder.prismaNode('Form', {
	...
	fields: (t) => ({
		...
		event: t.relation('event', {
			nullable: true,
			description: 'Événement associé au formulaire',
		}),
		canEdit: t.boolean({
			...
			resolve: async (query, { id: formId }, { id }) =>
				prisma.formSection.findFirstOrThrow({
					...
					where: {
						formId,
						id: id
							? ensureHasIdPrefix(id, 'FormSection')
							: undefined,
					},
				}),
		}),
		...
		hasSections: t.boolean({
			...
			resolve: async ({ id }) =>
				(await prisma.formSection.count({
					where: { formId: id, title: { not: '' } },
				})) > 0,
		}),
		checkboxesAreEnabled: t.exposeBoolean(
			'enableAnswersCompletionCheckbox',
		),
	}),
});
