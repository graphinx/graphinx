...
import {
	...
	AnswersExportFormatsType,
} from '../types/answers-export-format.js';
	...
	t.string({
		...
		async resolve({ id }, { format }, { user }) {
			...
			const questions = uniqBy(
				...
				(a, b) =>
					a.section.order - b.section.order || a.order - b.order,
			);
			...
			for (const [answererId, answers] of Object.entries(answersByUser)) {
				const answerer = answers.find(
					(a) => a.createdById === answererId,
				)?.createdBy;

				tabularData.push({
					Personne: answerer ? fullName(answerer) : '(Anonyme)',
					'Profil Churros': answerer
						? `https://churros.inpt.fr/@${answerer.uid}`
						: '',
					...
					...Object.fromEntries(
						questions.map((q) => {
							...
							return [
								q.title,
								answer ? answerToString(answer, user) : '',
							];
						}),
					),
				});
