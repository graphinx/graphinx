...
function groupAndSort<T extends Answer & { createdBy: User | null }>(
	answers: T[],
) {
	return Object.values(
		groupBy(answers.filter(propertyIsNotNull('createdBy')), 'createdById'),
	)
		.map((answers) => ({
			user: answers[0]!.createdBy,
			answers,
			date: new Date(
				Math.max(...answers.map((a) => a.updatedAt.valueOf())),
			),
		}))
		.sort((a, b) => b.date.valueOf() - a.date.valueOf());
}

function propertyIsNotNull<
	T extends Record<string, unknown>,
	K extends keyof T,
>(property: K) {
	function _notNull(
		result: T,
	): result is T & { [key in K]: NonNullable<T[K]> } {
