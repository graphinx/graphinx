...
builder.mutationField('upsertQuestion', (t) =>
	t.prismaFieldWithInput({
		...
		validate: [
			[
				({ input: { formId, sectionId, id } }) =>
					Boolean(formId || sectionId || id),
				{
					message:
						'Il faut fournir un formulaire ou une section pour créer une question',
				},
			],
			[
				({ input: { type, scale } }) =>
					type !== 'Scale' || Boolean(scale),
				{
					message:
			...
			[
				({ input: { type, options } }) =>
					type.startsWith('Select') || !options,
				{
					message:
			...
			[
				({ input: { type, allowedFiletypes } }) =>
					type === 'FileUpload' || !allowedFiletypes,
				{
					message:
			...
			[
				({ input: { type, allowOptionOther } }) =>
					type === 'SelectOne' || !allowOptionOther,
				{
					message:
		...
		async resolve(
			...
			const { answer: defaultAnswer } = castAnswer(
				...
				{
					...input,
					scaleStart: scale?.minimum ?? null,
					scaleEnd: scale?.maximum ?? null,
				},
				user,
			);
			const data = {
				...
				options:
					...
					(scale
						? [scale?.minimumLabel ?? '', scale?.maximumLabel ?? '']
						: undefined),
				defaultAnswer,
			};
			...
			if (formId && sectionId) {
				...
				if (
					!sectionsOfForm.some((section) => section.id === sectionId)
				) {
