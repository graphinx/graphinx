...
builder.mutationField('answerFormSection', (t) =>
	t.prismaField({
		...
		validate: [
			[
				({ answers }) =>
					answers.length === uniqBy(answers, 'question').length,
				{
					message: 'Il y a plusieurs réponses pour la même question',
					path: ['answers'],
				},
			],
			[
		...
		async resolve(query, _, { answers, section: sectionId }, { user }) {
			if (!user)
				throw new GraphQLError(
					'Vous devez être connecté pour répondre à un formulaire',
				);
			if (answers.length === 0) return [];
			const questions = await prisma.question.findMany({
				where: {
					sectionId,
					id: { in: answers.map((a) => a.question) },
				},
			});
			...
			return answers.map(
				({ question }) =>
					results.find((result) => result.questionId === question)!,
			);
		},
	}),
);
