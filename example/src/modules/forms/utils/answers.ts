...
export function castAnswer(
	...
	if (type === 'Scale') {
		...
		if (Number.isNaN(ans))
			throw new GraphQLError("Réponse invalide: ce n'est pas un nombre");
		if (scaleStart === null || scaleEnd === null)
			throw new GraphQLError(
				'Échelle invalide: les bornes ne sont pas définies',
			);
...
export function normalizeScaleAnswer(answer: string[]): undefined | number {
	...
	const [current, total] = answer[0]!
		.split('/', 2)
		.map((part) => Number.parseInt(part)) as [number, number];
