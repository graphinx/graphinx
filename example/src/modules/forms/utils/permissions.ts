...
export const requiredIncludesForPermissions = {
	...
	event: {
		include: {
			...
			coOrganizers: true,
			group: {
				include: { studentAssociation: { include: { school: true } } },
			},
			tickets: true,
		},
	},
...
export function canEditForm(
	...
	if (
		associatedEvent &&
		userCanManageEvent(associatedEvent, user, { canEdit: true })
	)
		return true;
	return false;
}
...
export function canCreateForm(
	...
	if (
		...
		return false;
	if (associatedGroup && !userIsOnBoardOf(user, associatedGroup.uid))
		return false;
	return true;
}
...
export function canSeeAllAnswers(
	...
	if (
		form.group &&
		user.groups.some(
			(g) => g.group.uid === form.group?.uid && g.canScanEvents,
		)
	)
		return true;
	return false;
...
export function canAnswerForm(
	...
	userContributions: Array<
		Contribution & {
			option: ContributionOption & { paysFor: StudentAssociation[] };
		}
	>,
) {
	if (user?.admin) return true;
	...
	if (form.contributorsOnly) {
		if (!form.group)
			throw new GraphQLError(
				'Cannot have a contributorsOnly form without a group',
			);

		if (
			!userContributions.some((c) =>
				c.option.paysFor.some(
					(p) => p.id === form.group?.studentAssociationId,
				),
			)
		) {
			console.error(
...
export function canModifyFormAnswers(
	...
	if (!canAnswerForm(form, associatedEvent, user, userContributions))
		return false;
	return true;
}
...
export function canSeeForm(
	form: {
		...
		group: null | {
			id: string;
			studentAssociation: null | { schoolId: string };
		};
	},
	associatedEvent:
		| (Parameters<typeof userCanAccessEvent>[0] &
				Parameters<typeof userCanManageEvent>[0])
		| null,
	user: Context['user'],
) {
	...
	if (
		...
		!user?.major?.schools.some(
			(s) => form.group?.studentAssociation?.schoolId === s.id,
		)
	)
		return false;
...
export function canSeeAnswerStats(
	form: Form & {
		group: Group | null;
		sections: Array<FormSection & { questions: Question[] }>;
	},
	associatedEvent: Parameters<typeof userCanManageEvent>[0] | null,
	user: Context['user'],
) {
...
export function isOpenForAnswers(form: Pick<Form, 'opensAt' | 'closesAt'>) {
	return isWithinPartialInterval(new Date(), {
		start: form.opensAt,
		end: form.closesAt,
	});
}
...
export function hasAnonymousQuestions(form: {
	...
	return form.sections.some((section) =>
		section.questions.some((q) => q.anonymous),
	);
}
