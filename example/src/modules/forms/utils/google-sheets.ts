...
function rowRange(questionsCount: number): string {
	return `${ANSWERS_SHEET_NAME}!A1:${String.fromCodePoint(
		'A'.codePointAt(0)! + questionsCount,
	)}999`;
}
...
export async function removeAnswersRowsForUser(
	...
	try {
		...
		const rowsToDelete = rows.filter(
			(row) => row[0]?.trim().toLowerCase() === userId,
		);
		...
		const sheetId = sheet.data.sheets?.find(
			(s) => s.properties?.title === ANSWERS_SHEET_NAME,
		)?.properties?.sheetId;
...
export async function appendFormAnswersToGoogleSheets(
	...
	const answerer = await prisma.user.findUniqueOrThrow({
		where: { id: userId },
	});
	...
	const latestAnswerDate = new Date(
		Math.max(
			...questions.flatMap(({ answers }) =>
				answers.map((a) => a.createdAt.valueOf()),
			),
		),
	);
	...
	await removeAnswersRowsForUser(
		form.linkedGoogleSheetId,
		sheets,
		userId,
		questions.length,
