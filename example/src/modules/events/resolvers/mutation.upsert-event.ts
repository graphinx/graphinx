	...
	t.prismaField({
		...
		args: {
			...
			coOrganizers: t.arg.stringList({
				description: 'List of group uids',
			}),
		...
		async authScopes(_, { id, groupUid }, { user }) {
			...
			const event = await prisma.event.findUniqueOrThrow({
				...
				include: {
					managers: true,
					group: { select: { studentAssociationId: true } },
				},
			});
		...
		async resolve(
			...
			if (frequency !== EventFrequency.Once && tickets.length > 0)
				throw new GraphQLError(
					'Events with a frequency cannot have tickets',
				);
			...
			const connectFromListOfUids = (uids: string[]) =>
				uids.map((uid) => ({ uid }));
			const connectFromListOfIds = (ids: string[]) =>
				ids.map((id) => ({ id }));
			...
			const event = await prisma.event.upsert({
				...
				update: {
					...
					managers:
						user?.admin ||
						oldEvent?.managers.some(
							(m) =>
								m.userId === user?.id && m.canEditPermissions,
						)
							? {
									deleteMany: {
										userId: {
											notIn: managersWithUserId.map(
												(m) => m.userId,
											),
										},
									},
			...
			const simppsPromotion = await prisma.promotion.findFirst({
				where: {
					type: PrismaTypes.PromotionType.SIMPPS,
			...
			for (const ticket of tickets) {
				const ticketGroupId = ticket.groupName
					? ticketGroups.find((tg) => tg.name === ticket.groupName)!
							.id
					: undefined;
				await prisma.ticket.upsert({
					...
					create: {
						...
						id: undefined,
						group: ticketGroupId
							? { connect: { id: ticketGroupId } }
							: undefined,
						event: { connect: { id: event.id } },
						links: { create: ticket.links },
						// connections
						openToGroups: {
							connect: connectFromListOfUids(ticket.openToGroups),
						},
						openToSchools: {
							connect: connectFromListOfUids(
								ticket.openToSchools,
							),
						},
						openToMajors: {
							connect: connectFromListOfIds(ticket.openToMajors),
						},
						autojoinGroups: {
							connect: connectFromListOfUids(
								ticket.autojoinGroups,
							),
						},
						// SIMPPS promotion
						subjectToPromotions:
							simppsPromotion &&
							group.type === GroupType.StudentAssociationSection
			...
			if (finalEvent.includeInKiosk || oldEvent?.includeInKiosk) {
				...
				for (const major of majorsToReload) {
					...
					pubsub.publish(
						subscriptionName('KioskReload', 'updated', major.uid),
					);
				}
			}
