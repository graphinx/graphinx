	...
	t.prismaField({
		...
		async resolve(query, _, { today }, { user }) {
			...
			function isRecurrentEventVisible(event: Event): boolean {
				...
				if (
					startOfWeek(event.startsAt, { weekStartsOn: 1 }) >
					startOfWeek(today, { weekStartsOn: 1 })
				)
					return false;

				if (event.recurringUntil)
					return isBefore(today, event.recurringUntil);
				return true;
			}
			...
			function fixRecurrentEventDates(event: Event): Event {
				...
				switch (frequency) {
					case EventFrequency.Weekly: {
						const todayWeek = setDay(today, getDay(startsAt), {
							weekStartsOn: 1,
						});
					...
					case EventFrequency.Biweekly: {
						...
						const weekDelta = differenceInWeeks(
							todayWeek,
							startsAt,
						);
						if (weekDelta % 2 === 0) {
					...
					case EventFrequency.Monthly: {
						const monthCorrect =
							getMonth(today) ===
							getMonth(endOfWeek(today, { weekStartsOn: 1 }))
								? 0
								: 1;
						const yearCorrect =
							getYear(today) ===
							getYear(endOfWeek(today, { weekStartsOn: 1 }))
								? 0
								: 1;

						startsAt = setMonth(
							startsAt,
							getMonth(today) + monthCorrect,
						);
						endsAt = setMonth(
							endsAt,
							getMonth(today) + monthCorrect,
			...
			if (!user) {
				return prisma.event
					...
					.then((events) =>
						events
							.filter((element) =>
								isRecurrentEventVisible(element),
							)
