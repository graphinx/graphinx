		...
		`OffsetConnection:${
			typeof dateOrCursor === 'string'
				...
				: isValid(dateOrCursor)
					? formatISO(dateOrCursor as Date, {
							representation: 'date',
						})
					: ''
		}`,
...
export const EventsByDayType = builder
	.objectRef<EventsByDay>('EventsByDay')
	.implement({
		fields: (t) => ({
			happening: t.field({
				type: [EventType],
				...
				resolve: ({ date, events }) =>
					events.filter(({ startsAt }) => isSameDay(startsAt, date)),
			}),
			shotgunning: t.field({
				type: [EventType],
				description:
				...
				resolve: ({ date, events }) =>
					events.filter(({ tickets }) => {
						const earliestOpenTime = soonest(
							...tickets.map((t) => t.opensAt).filter(notNull),
						);
