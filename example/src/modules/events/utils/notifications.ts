...
export async function scheduleShotgunNotifications(
	...
	const opensAt = new Date(
		Math.min(
			...tickets.map(
				({ opensAt }) => opensAt?.valueOf() ?? Number.POSITIVE_INFINITY,
			),
		),
	);
	...
	const closesAt = new Date(
		Math.min(
			...tickets.map(
				({ closesAt }) =>
					closesAt?.valueOf() ?? Number.POSITIVE_INFINITY,
			),
		),
	);
		...
		async (
			...
			const schoolsOfEvent = new Set(
				[
					...
					...event.coOrganizers.map(
						(c) => c.studentAssociation?.school,
					),
				]
					.filter(Boolean)
					.map((s) => s!.id),
			...
			if (
				...
				!user.major?.schools.some((school) =>
					schoolsOfEvent.has(school.id),
				)
			)
				return;
			...
			if (
				event.visibility === Visibility.Unlisted ||
			...
			switch (type) {
				case 'Opening': {
					...
					notification.body = `Prépare-toi, il ouvre à ${format(
						opensAt,
						'HH:mm',
					)}`;
				...
				case 'Closing': {
					notification.title = `Shotgun pour ${event.title}`;
					notification.body = `Attention, il ferme à ${format(
						closesAt,
						'HH:mm',
					)}, dépeches-toi !`;
			...
			return {
				...
				async afterSent() {
					console.info(
						`[${new Date()}] Set notifiedAt on ${event.id}`,
					);
