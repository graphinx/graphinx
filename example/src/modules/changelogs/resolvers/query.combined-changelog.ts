...
This is way more useful for querying a range of versions for a changelog, but not the usual way ranges are defined.`,
		...
		args: {
			...
			from: t.arg.string({
				...
				validate: {
					refine: (value) =>
						Boolean(SemVer.valid(value, { loose: true })),
				},
			}),
			to: t.arg.string({
		...
		async resolve(_, { from, to, sort }, { user }) {
			if (!from) {
				...
				const { latestVersionSeenInChangelog } =
			...
			const selectedReleases = changelog.releases
				.filter(
					(release) =>
						release.version &&
						SemVer.gt(release.version, from!) &&
						SemVer.lte(release.version, to),
				)
			...
			if (selectedReleases.length === 0)
				throw new GraphQLError(
					`Aucune version entre ${from} et ${to}.`,
				);
