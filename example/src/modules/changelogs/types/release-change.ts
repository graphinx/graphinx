...
export const ReleaseChangeType = builder
	.objectRef<ReleaseChange>('ReleaseChange')
	.implement({
		description: 'A change in the changelog',
		fields: (t) => ({
			text: t.exposeString('text', {
				description: 'The text of the change',
			}),
			html: t.string({
				async resolve({ text }) {
					return toHtml(text, {
						linkifyUserMentions: true,
						linkifyGitlabItems: true,
					});
				},
			}),
