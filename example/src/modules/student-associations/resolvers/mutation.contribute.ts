	...
	t.field({
		...
		async resolve(_, { optionId, phone }, { user }) {
			...
			const contributionOption =
				await prisma.contributionOption.findUnique({
					where: { id: optionId },
					include: {
						beneficiary: true,
						offeredIn: { include: { majors: true } },
					},
				});

			if (!contributionOption)
				throw new GraphQLError('Option de cotisation introuvable');
			...
			if (
				!contributionOption.offeredIn.majors.some(
					(major) => user.major?.id === major.id,
				)
			)
				throw new GraphQLError(
					"Cette option de cotisation n'est pas offerte à votre école",
				);

			// eslint-disable-next-line prefer-const
			let { transaction, ...contribution } =
