...
import {
	canEditGroupPages,
	canEditStudentAssociationPages,
...
builder.prismaObjectField(StudentAssociationType, 'canEditPages', (t) =>
	t.boolean({
		...
		resolve: ({ id }, _, { user }) =>
			canEditStudentAssociationPages(user, id),
	}),
);
...
builder.mutationField('upsertPage', (t) =>
	t.prismaField({
		...
		validate: [
			[
				({ group, studentAssociation }) =>
					Boolean(group || studentAssociation),
				{
					message:
		...
		async resolve(query, _, input, { user }) {
			...
			if (!studentAssociation && !group)
				throw new GraphQLError(
					'Groupe ou association étudiante introuvable.',
				);
			...
			const result = await prisma.page.upsert({
				...
				update: {
					...
					...(input.studentAssociation
						? {}
						: { studentAssociation: { disconnect: true } }),
				},
			});
