...
export async function removeMemberFromGroupMailingList(
	groupId: string,
	email: string,
) {
...
export async function updateMemberBoardLists(
	...
	const { studentAssociation, studentAssociationId } =
	...
	const { allPrezMailingList, allTrezMailingList, allBoardMailingList } =
		studentAssociation;
...
export async function addMemberToGroupMailingList(
	groupId: string,
	email: string,
) {
	const { mailingList } = await prisma.group.findUniqueOrThrow({
