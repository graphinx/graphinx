...
builder.mutationField('paidShopPayment', (t) =>
	t.prismaField({
		...
		async authScopes(_, { shopPaymentId }, { user }) {
			...
			if (!shopPayment)
				throw new GraphQLError("La commande n'existe pas");
		...
		async resolve(
			query,
			_,
			{ shopPaymentId, paymentMethod, phone },
			{ user },
		) {
			...
			if (
				user?.admin ||
				userIsOnBoardOf(user, shopPayment.shopItem.group.uid)
			) {
				...
				shopPayment.shopItem.max === 0
					...
					: shopPayment.shopItem.max -
						shopPayment.shopItem.shopPayments.reduce(
							(acc, curr) =>
								acc +
								(curr.userId === user.uid ? curr.quantity : 0),
							0,
						);
