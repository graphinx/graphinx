...
builder.mutationField('upsertShopPayment', (t) =>
	t.prismaField({
		...
		async authScopes(_, { shopItemId }, { user }) {
			...
			switch (shopItem.visibility) {
				...
				case Visibility.SchoolRestricted: {
					if (
						shopItem.group.studentAssociation?.school.uid ===
						user.schoolUid
					)
						break;
		...
		async resolve(
			...
			if (quantity === 0)
				throw new GraphQLError("You can't buy 0 of something");
			...
			const userLeft =
				...
				shopItem.shopPayments.reduce(
					(acc, curr) =>
						acc + (curr.userId === userUid ? curr.quantity : 0),
					0,
				);

			if (stockLeft < quantity && shopItem.stock != 0)
			...
			for (const [i, option] of Object.entries(shopOptions)) {
				...
				if (
					...
					!option.options.includes(
						answers[Number.parseInt(i)] as string,
					)
				) {
			...
			if (answers.some((answer) => answer.length > 255))
				throw new GraphQLError('Texte trop long');
