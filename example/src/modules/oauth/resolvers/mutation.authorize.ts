...
- \`redirect_uri\`: The redirect URI used in this request
      `,
		...
		async resolve(_, { clientId, redirectUri }, { user }) {
			...
			if (
				!client.allowedRedirectUris.some(
					(uri) => normalizeUrl(redirectUri) === normalizeUrl(uri),
				)
			) {
				const message = `Invalid redirect URI, must be one of ${client.allowedRedirectUris
