...
builder.mutationField('editApp', (t) =>
	t.prismaField({
		...
		async resolve(query, _, { id, ...data }, { user }) {
			...
			const {
				allowedRedirectUris: oldAllowedRedirectUris,
				website: oldWebsite,
			} = await prisma.thirdPartyApp.findUniqueOrThrow({ where: { id } });

			data.allowedRedirectUris = data.allowedRedirectUris?.map((uri) =>
				normalizeUrl(uri),
			);
			...
			const allowedURIsWillChange =
				...
				!(
					oldAllowedRedirectUris.every((uri) =>
						data.allowedRedirectUris!.includes(uri),
					) &&
					oldAllowedRedirectUris.length ===
						data.allowedRedirectUris.length
				);

			// eslint-disable-next-line unicorn/no-null
			const websiteWillChange = ![undefined, null, oldWebsite].includes(
				data.website,
			);
			...
			return prisma.thirdPartyApp.update({
				...
				data: {
					...
					owner: data.ownerGroupUid
						? { connect: { uid: data.ownerGroupUid } }
						: undefined,
