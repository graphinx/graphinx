...
export async function checkLydiaTransaction(transaction: LydiaTransaction) {
	if (!transaction.requestId)
		throw new GraphQLError(
			'Aucune requête pour cette transaction, impossible de checker',
		);
...
export async function sendLydiaPaymentRequest(
	...
	const response = await fetch(
		`${PUBLIC_LYDIA_API_URL}/api/request/do.json`,
		{
			method: 'POST',
	...
	if (!response.ok)
		throw new Error('Error while sending the payment request');
