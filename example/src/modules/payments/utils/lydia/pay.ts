...
export async function payEventRegistrationViaLydia(
	...
	const beneficiaryVendorToken =
		registration.ticket.event.beneficiary?.vendorToken;
	if (!beneficiaryVendorToken)
		throw new GraphQLError("L'évènement n'a pas de bénéficiaire");

	// Check if transaction was already paid for, in that case mark registration as paid
	if (
		registration.lydiaTransaction?.requestId &&
		registration.lydiaTransaction.requestUuid
	) {
		const state = await checkLydiaTransaction(
			registration.lydiaTransaction,
		);
		if (state === LydiaTransactionState.Paid) {
			await prisma.logEntry.create({
				data: {
					...
					area: 'lydia',
					message:
						'Transaction was already paid for, marking registration as paid',
	...
	const requestDetails = await sendLydiaPaymentRequest(
		...
		await priceWithPromotionsApplied(
			registration.ticket,
			registration.author ?? undefined,
		),
		phone,
...
export async function cancelLydiaTransaction(
	transaction: LydiaTransaction,
	vendorToken: string,
) {
	if (!transaction.requestId)
		throw new GraphQLError(
			"Aucune requête pour cette transaction, impossible de l'annuler",
		);
...
export async function payShopPaymentViaLydia(
	...
	if (!shopPayment.shopItem.lydiaAccount)
		throw new Error('Lydia account not found');
	// Check if transaction was already paid for, in that case mark registration as paid
	if (
		shopPayment.lydiaTransaction?.requestId &&
		shopPayment.lydiaTransaction.requestUuid
	) {
