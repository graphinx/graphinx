...
const {
	...
	PUBLIC_PAYPAL_API_BASE_URL,
} = process.env;

async function accessToken() {
	const response = await fetch(
		new URL(`/v1/oauth2/token`, PUBLIC_PAYPAL_API_BASE_URL),
		{
			method: 'POST',
			headers: {
...
export async function initiatePaypalPayment(
	...
	const response = await fetch(
		new URL('/v2/checkout/orders', PUBLIC_PAYPAL_API_BASE_URL),
		{
			method: 'POST',
			headers: {
	...
	if (!response['id'])
		console.error(
			`Error while creating PayPal order: ${JSON.stringify(response)}`,
		);
