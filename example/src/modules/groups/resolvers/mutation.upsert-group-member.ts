				...
				userIsAdminOf(user, studentAssociation) ||
					...
					user?.groups.some(
						({ group, canEditMembers }) =>
							canEditMembers && group.id === groupId,
					),
			);
		},
		async resolve(
			...
			const quittingBoard =
				onBoard(oldMember) &&
				!onBoard({ president, treasurer, vicePresident, secretary });
			...
			const rolesText = (
				member: Record<(typeof boardKeys)[number], boolean>,
			) =>
				(boardKeys.some((k) => member[k])
					? boardKeys.filter((k) => member[k])
					: ['(aucun)']
				).join(', ');
