...
builder.mutationField('upsertGroup', (t) =>
	t.prismaField({
		...
		async resolve(
			...
			if (
				membersNeedToPayForTheStudentAssociation({ type }) &&
				!studentAssociationUid
			)
				throw new GraphQLError(
					"Il faut préciser l'AE de rattachement pour un club ou une liste",
				);
			...
			if (
				...
				if (!newParent)
					throw new GraphQLError('uid de groupe parent invalide');
				...
				if (oldGroup) {
					if (
						hasCycle(
							allGroups.map((g) =>
								g.id === oldGroup.id
									? { ...oldGroup, parentId: newParent.id }
									: g,
							),
						)
					)
						throw new GraphQLError(
							'La modification créerait un cycle dans les groupes',
						);
					...
					newParent.id &&
					hasCycle([{ parentId: newParent.id, id: '' }, ...allGroups])
				) {
					throw new GraphQLError("Can't create a cycle");
				}
			}

			if (parentUid === oldGroup?.uid)
				throw new GraphQLError('Group cannot be its own parent');

			if (oldGroup) {
				const oldStudentAssociation =
					await prisma.studentAssociation.findUnique({
				...
				if (
					...
					(oldGroup?.type != type ||
						(oldStudentAssociation &&
							oldStudentAssociation.uid != studentAssociationUid))
				)
					// Non admin users aren't allowed to change attached ae and group type
					throw new GraphQLError(
						"Vous n'êtes pas autorisé à modifer ces paramètres.",
					);
			}
