...
export function userIsOnGroupBoard(
	user: Context['user'],
	group: { uid: string } | { id: string },
) {
	return user?.groups.some(
		(membership) =>
			groupsAreTheSame(group, membership.group) && onBoard(membership),
	);
}
...
function groupsAreTheSame<G extends { id: string } | { uid: string }>(
	...
	if ('uid' in group && 'uid' in otherGroup)
		return otherGroup.uid === group.uid;
	if ('id' in group && 'id' in otherGroup) return otherGroup.id === group.id;
	throw new Error(
		'Invalid group comparison: one group has an ID and the other has a UID',
	);
}
 ...
 */
function equalOrBothNotProvided<T>(
	a: T | null | undefined,
	b: T | null | undefined,
): boolean {
