...
export async function scheduleNewArticleNotification(
	...
	return scheduleNotification(
		async (user) => {
			...
			if (
				article.visibility === Visibility.Unlisted ||
			...
			if (
				...
				!user.major?.schools.some(
					(school) =>
						school.id ===
						article.group.studentAssociation?.school.id,
				)
			)
				return;
			...
			if (article.visibility === Visibility.GroupRestricted) {
				...
				const ancestors = await prisma.group
					...
					.then((groups) =>
						mappedGetAncestors(groups, user.groups, {
							mappedKey: 'groupId',
						}),
					)
				...
				if (!ancestors.some(({ uid }) => uid === article.group.uid))
					return;
			}
			...
			return {
				...
				async afterSent() {
					console.info(
						`[${new Date().toISOString()}] Setting notifiedAt for article ${
							article.id
						}`,
					);
