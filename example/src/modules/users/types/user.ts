...
export const UserType = builder.prismaNode('User', {
	...
	fields: (t) => ({
		...
		latestVersionSeenInChangelog: t.exposeString(
		...
		adminOf: t.boolean({
			...
			args: {
				studentAssociation: t.arg.string({
					description: "UID de l'association étudiante",
				}),
			},
			resolve: async ({ id }, { studentAssociation }) => {
				...
				return user.adminOfStudentAssociations.some(
					(a) => a.uid === studentAssociation,
				);
			},
		}),
		...
		studentAssociationAdmin: t.boolean({
			...
			resolve: ({ adminOfStudentAssociations }) =>
				adminOfStudentAssociations.length > 0,
		}),
		canEditGroup: t.boolean({
			...
			resolve: async ({ id }, { uid: groupUid }) => {
				...
				return (
					(user.adminOfStudentAssociations?.some(
						(a) => a.id === studentAssociationId,
					) ??
						false) ||
					(user.canEditGroups?.some(
						(a) => a.id === studentAssociationId,
					) ??
						false) ||
					user.admin
		...
		canAccessDocuments: t.boolean({
			resolve: ({ admin, canAccessDocuments }) =>
				admin || canAccessDocuments,
