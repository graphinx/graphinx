...
export function prismaUserFilterForStudentAssociationAdmins(
...
export function prismaGroupFilterForStudentAssociationAdmins(
	user: NonNullable<Context['user']>,
) {
	if (user.admin) return {};
	return {
...
export function canBeEdited(
	user: {
		id: string;
		major: {
			schools: Array<{ studentAssociations: Array<{ id: string }> }>;
		} | null;
	},
	...
	return (
		...
		me.adminOfStudentAssociations.some((a) =>
			user.major?.schools.some((s) =>
				s.studentAssociations.some((sa) => sa.id === a.id),
			),
		)
	);
}
