...
builder.mutationField('upsertGodparentRequest', (t) =>
	t.prismaField({
		...
		async authScopes(_, { godchildUid }, { user }) {
			...
			if (godchildUid !== user?.uid)
				return Boolean(userIsAdminOf(user, studentAssociations));
			return Boolean(user);
		},
		async resolve(query, _, { id, godparentUid, godchildUid }, { user }) {
			if (!user) throw new GraphQLError('Not logged in');
			const godparent = await prisma.user.findUniqueOrThrow({
				where: { uid: godparentUid },
			});
			const godchild = await prisma.user.findUniqueOrThrow({
				where: { uid: godchildUid },
			});

			const family = await getFamilyTree({
				id: godchild.id,
				godparentId: godparent.id,
			});
