...
builder.mutationField('updateUser', (t) =>
	t.prismaField({
		...
		async resolve(
			...
			const userIsAdmin = userIsAdminOf(
				user,
				objectValuesFlat(targetUser.major),
			);
			...
			if (changingEmail) {
				...
				if (existingUser)
					throw new GraphQLError('Cet e-mail est déjà utilisé');
			...
			if (!userIsAdmin && changingGraduationYear)
				throw new GraphQLError(
					'Demande au bureau de ton AE pour changer de promo',
				);
