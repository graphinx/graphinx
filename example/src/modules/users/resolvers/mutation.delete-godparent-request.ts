...
builder.mutationField('deleteGodparentRequest', (t) =>
	t.prismaField({
		...
		async authScopes(_, { id, accept }, { user }) {
			...
			if (userIsAdminOf(user, objectValuesFlat(request.godparent)))
				return true;
		...
		async resolve(_query, _, { id, accept }) {
			...
			if (accept) {
				...
				await notify([request.godchild], {
					body: `${fullName(
						request.godparent,
					)} a accepté votre demande de parrainage!`,
				...
				await notify([request.godchild], {
					body: `${fullName(
						request.godparent,
					)} a refusé votre demande de parrainage.`,
