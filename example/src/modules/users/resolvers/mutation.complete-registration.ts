...
builder.mutationField('completeRegistration', (t) =>
	t.field({
		type: builder.unionType('CompleteSignupResult', {
			...
			resolveType: (value) =>
				Object.hasOwn(value, 'usingQuickSignup')
					? UserCandidateType
					: UserType,
		}),
		...
		async resolve(
			...
			const user = await completeRegistration(
				...
				if (
					...
					throw new GraphQLError(
						"Cette demande d'inscription déjà complétée ou inexistante.",
					);

				throw error;
			});
			const userOrCandidate: (typeof user | UserCandidate) & {
			...
			await notify(adminsResponsibleForThisSignup, {
				...
				body:
					`${userOrCandidate.email} (${userOrCandidate.firstName} ${
						userOrCandidate.lastName
					}, ${
						userOrCandidate.graduationYear
							? yearTier(userOrCandidate.graduationYear)
							: '?'
					}A ${
						userOrCandidate.major?.shortName ?? 'sans filière'
					}) ` +
					(needsVerification
				...
				data: {
					...
					goto: needsVerification
						? `/signups/edit/${userOrCandidate.email}`
						: `/@${user.uid}`,
					group: undefined,
				},
			});
