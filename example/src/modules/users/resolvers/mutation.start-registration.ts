...
builder.mutationField('startRegistration', (t) =>
	t.field({
		...
		resolve: async (_, { email, quickSignupCode }) =>
			register(email, quickSignupCode ?? undefined),
	}),
);
...
export const register = async (
	...
	const { token } = schoolUser
		? await prisma.userCandidate.upsert({
				...
				create: {
					...
					graduationYear:
						schoolUser.graduationYear ?? fromYearTier(0),
				},
				update: {
					...schoolUser,
					major: major ? { connect: { id: major.id } } : undefined,
	...
	await prisma.logEntry.create({
		data: {
			action: 'start',
			area: 'signups',
			message: `Inscription de ${email} démarrée. schoolUser: ${JSON.stringify(
				schoolUser,
			)}`,
			target: `token ${token}`,
		},
