	...
	t.withAuth({ student: true }).prismaField({
		...
		async resolve(query, _, { now, activeOnly, width }, { user }) {
			...
			const usersNonflat = await Promise.all(
				dateRangeAround(now, width).flatMap(async (d) =>
					usersBornOn(d),
				),
			);
					...
					major: {
						schools: {
							some: {
								uid: {
									in:
										user.major?.schools.map((s) => s.uid) ??
										[],
									not: 'inp',
								},
							},
			...
			if (activeOnly)
				return users.filter(({ graduationYear }) =>
					[1, 2, 3].includes(yearTier(graduationYear)),
				);
			return users;
		},
	}),
