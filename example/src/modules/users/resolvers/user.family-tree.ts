...
builder.objectField(UserType, 'familyTree', (t) =>
	t.field({
		type: FamilyTree,
		resolve: ({ id, godparentId }) =>
			getFamilyTree({ id, godparentId: godparentId ?? undefined }),
	}),
);
...
export async function getFamilyTree({
	...
	while ((parentOfUltimate = await parentId(ultimateParentId)))
		ultimateParentId = parentOfUltimate;
	...
	async function gather(rootUid: string): Promise<Nesting> {
		...
		return [
			rootUid,
			await Promise.all(
				parent.godchildren.map(async (u) => gather(u.uid)),
			),
		];
	}
