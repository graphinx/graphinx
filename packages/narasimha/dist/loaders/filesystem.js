        ...
        for (const name of index) {
            ...
            for (const filepath of await typescriptFilesWithoutBarrels(path.join(directory, 'resolvers'))) {
                ...
                if (filename.startsWith('subscription')) {
                    let fieldname = kebabToCamel(filename.replace(/^subscription\./, '').replace(/\.ts$/, ''));
                    includedItems.add(fieldname);
                    sourcemap.set(fieldname, { filepath });
                }
            }
            if (includedItems.size === 0) {
                console.warn(`WARN: ${directory} has no types nor resolvers. Files found...\n\tIn ${path.join(directory, 'types')}: ${(await typescriptFilesWithoutBarrels(path.join(directory, 'types')))
                    .map(f => path.basename(f))
                    .join(', ')}\n\tIn ${path.join(directory, 'resolvers')}: ${(await typescriptFilesWithoutBarrels(path.join(directory, 'resolvers')))
                    .map(f => path.basename(f))
                    .join(', ')}`);
            }
            modules.push({
...
async function readdirNotExistOk(directory) {
    if (!(await stat(directory).catch(() => false))) {
...
async function typescriptFilesWithoutBarrels(directory) {
    return (await readdirNotExistOk(directory)).filter(file => file.endsWith('.ts') && !file.endsWith('.d.ts') && path.basename(file) !== 'index.ts');
}
