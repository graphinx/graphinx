<script>import { Kind, Pantry } from "../index.js";
export let typ;
export let pantry;
...
function willExpandEnum(t) {
  const valuesCount = t.kind === Kind.Enum ? enumType?.enumValues.length : t.kind === Kind.Union ? unionType?.possibleTypes.length : 0;
  return Boolean(
    (t.kind === "ENUM" || t.kind === "UNION") && !noExpandEnums && (inline ? (valuesCount ?? 0) <= 3 : true) && (valuesCount ?? 0) <= 10
  );
}
</script>
