...
import { liveIndicatorSettings } from "../global.js";
export let field;
export let pantry;
export let kind;
...
function syntaxHighlightTypeName(t) {
  if (t.kind === "ENUM")
    return "enum";
  return t.name?.toLowerCase() ?? (t.ofType ? syntaxHighlightTypeName(t.ofType) : "Unknown").toLowerCase();
}
function expandTypedef(t) {
  const name = firstNonWrapperType(t)?.name ?? "";
  return name.includes("Input") || name.includes("HealthCheck");
}
function firstNonWrapperType(t) {
