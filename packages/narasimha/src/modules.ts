...
import { kebabToCamel, kebabToPascal } from "../../template/src/lib/casing.js";
import {
	getFrontmatter,
	markdownToHtml,
	type ResolverFromFilesystem,
} from "../../template/src/lib/markdown.js";
...
export async function getAllModules() {
	return (
		await Promise.all(
			(
				await readdir("../api/src/modules")
			).map(async (folder) => getModule(folder)),
		)
	)
		.filter(
		...
		.sort(
			(a, b) => MODULES_ORDER.indexOf(a.name) - MODULES_ORDER.indexOf(b.name),
		);
}
