...
const { cwd, useIntrospection, endpoint, authToken, ...options } = await p.group(
	{
		useIntrospection: () =>
			p.confirm({
		...
		name: () => p.text({ message: "What's the name of the project?", defaultValue: 'docs' }),
		eslint: () => p.confirm({ message: 'Use ESLint?', defaultValue: true }),
		prettier: () => p.confirm({ message: 'Use Prettier?', defaultValue: true }),
		types: () =>
			p.select({
...
if (useIntrospection) {
	try {
		...
		if ('errors' in response) {
			p.log.error(
				`Could not get the schema: ${red(response.errors.map(e => e.message).join('\n'))}`
			);
			process.exit(2);
		}
...
if (schema) {
	writeFileSync(`${cwd}/static/schema.json`, JSON.stringify(schema, null, '\t') + '\n');
...
copyFileSync(path.join(here, 'files', 'mdsvex.config.js'), path.join(cwd, 'mdsvex.config.js'));

const b = recast.types.builders;

/** @type {require('ast-types')} */
const svelteConfigFile = recast.parse(readFileSync(`${cwd}/svelte.config.js`, 'utf-8'), {
...
svelteConfig.init.properties.push(
	b.property(
		...
		b.arrayExpression([
			b.literal('.svelte'),
			b.spreadElement(b.memberExpression(b.identifier('mdsvexConfig'), b.identifier('extensions')))
		])
	)
);
