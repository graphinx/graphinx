...
import { type Module, NotFoundError, type PantryLoader, Pantry } from '@narasimha/core';
import { z } from 'zod';
				...
				sourceMapResolver: modules.some(m => m.sources)
					? (_, { name }) => {
							const filepath = Object.fromEntries(
								modules.flatMap(m => Object.entries(m.sources ?? {}))
							)[name];
...
} satisfies PantryLoader<{ path?: string }>;
