...
export async function getModule(
  ...
  if (config.modules.docs) {
    docs = await readFile(
      path.join(config._dir, config.modules.docs.replace("[module]", name)),
      "utf-8",
    );'[module]'
  }
  ...
  const itemIsInThisModule = (itemName: string) =>
    Boolean(items.find((i) => i.moduleName === name && i.name === itemName));
...
async function itemIsInModule(
  ...
  if (!result && item.type === "type" && item.inputTypeOf) {
    ...
    return itemIsInModule(items, config, module, mutationField, ...matchers);
  }
...
export async function moduleNames(
  ...
  if (subscriptionType)
    items = [...items, ...getAllFieldsOfType(schema, subscriptionType.name)];
...
export async function getAllItems(
  ...
  const analyzedItems: UncategorizedItem[] = items.map((schemaItem) => {
    let item = {
      ...
      deprecationReason: schemaItem.astNode
        ? getDirectiveCallArgument(schemaItem.astNode, "deprecated", "reason")
        : undefined,
      // sourceCodeURL:
  ...
  const staticMatchers = Object.fromEntries(
    await Promise.all(
      names.map(async (name) => {
        ...
        console.info(
          `\x1b[F\x1b[K\r🚂 Created static matcher for module ${b(name)}`,
        );
  ...
  const schemaMatchers = Object.fromEntries(
    await Promise.all(
      names.map(async (name) => {
        ...
        console.info(
          `\x1b[F\x1b[K\r🚂 Created schema matcher for module ${b(name)}`,
        );
        return [name, matcher] as const;
      }),
    ),
  ...
  const fsMatchers = Object.fromEntries(
    await Promise.all(
      names.map(async (name) => {
        const matcher = await createModuleFilesystemMatcher(config, name);
        console.info(
          `\x1b[F\x1b[K\r🚂 Created filesystem matcher for module ${b(name)}`,
        );
        return [name, matcher] as const;
      }),
    ),
